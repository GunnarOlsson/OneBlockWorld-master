 ...
 */
public class IntegerParsable implements ACParsable<Integer> {

    @Nullable
    @Override
    public Integer parse(@NotNull CommandSender sender, @NotNull String arg) {
        try {
    ...
    @Override
    public String filter(@NotNull CommandSender sender, @NotNull Integer parsed, @NotNull String filter) {
        ...
        for(String field : fields) {
            if(field.startsWith(RANGE_FILTER)) {
                ...
                try {
                    Integer min = Integer.parseInt(unparsed);
                    if(parsed < min)
                ...
                try {
                    Integer max = Integer.parseInt(unparsed);
                    if(parsed > max)
